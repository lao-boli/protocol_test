...
public class TopicNode {
    ...
    public TopicNode(String path, NodeType type, String indices, List<TopicNode> children, Set<Channel> channels, boolean containMulti, boolean containSingle) {
        this.path = path;
        this.type = type;
    ...
    public TopicNode addRoute(String path, Channel channel) {
        ...
        if (this.path.length() > 0 || !children.isEmpty()) {
            ...
            // 通配符节点直接插入
            if (c == '+' || c == '#') {
                this.insertChild(rpPath, channel);
                return this;
            }
     ...
     * @return 节点
     */
    public TopicNode removeRoute(String path, Channel channel) {
        ...
        if (node.getChildren().isEmpty()) {
            ...
            // 父节点为"+"节点,若"+"节点没有channel,则连同其一起删除
            // 因为"+"节点只可能有唯一一个子节点"/"
            if (NodeType.SINGLE.equals(parent.getType())) {
                if (parent.getChannels().isEmpty()) {
     ...
     */
    public void insertChild(String path, Channel channel) {
        ...
        if (i > -1) {
            if (i == 0) {
                ...
                insertStatic(path.substring(0, i)).insertChild(path.substring(i), channel);
                return;
            }
        }
     ...
     */
    private void insertSingle(String path, Channel channel) {
     ...
     */
    private void insertMulti(Channel channel) {
     ...
     * @return 只有路径的节点
     */
    private TopicNode insertStatic(String path) {
        TopicNode child = new TopicNode(path);
        child.setType(NodeType.STATIC);
        child.setParent(this);

        this.indices += path.substring(0, 1);
        children.add(child);
        return child;
    }
     ...
     * @return
     */
    private TopicNode insertCommon(String path, Channel channel) {
        TopicNode child = new TopicNode(path);
     ...
     */
    public void getValues(String path, Set<Channel> res) {
        // "#"中的channel匹配/a/,/a/#,
        ...
        if (this.containSingle) {
            ...
            String p = path.substring(Math.max(index, 0));
